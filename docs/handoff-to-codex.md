...
**Status:** SP1-SP9 complete | 18 tools | 209 tests passing | npm package live at v0.9.5 | Docker hardening patch v0.9.5 published | Docker install docs clarified
...
Public release `v0.9.5` is live from `Ryfter/canvas-design-studio`.

- npm package: `canvas-design-mcp@0.9.5`; `latest` points to `0.9.5`.
- GitHub Actions Publish run `25761059017` succeeded for tag `v0.9.5`: `test`, `publish-npm`, and `publish-docker` all completed successfully.
...
- `v0.9.4` published to npm but Docker failed before Buildx setup was added; `v0.9.5` supersedes it.
- Do not rerun the failed older tag workflows; publish any future fix as a new version/tag.

Private backup sync note: private `backup/master` must carry full docs and release notes, while public `origin/master` is the stripped release branch generated by `.\scripts\deploy-public.ps1`.

Read `docs/npm-publishing.md` before changing npm tokens, publish workflow, package metadata, or release tags.

---

## npm Publishing - 2026-05-13

Added `docs/npm-publishing.md` as the durable npm runbook for Claude/Codex:

- Confirms `canvas-design-mcp@0.9.5` is live and `latest`.
- Documents the public GitHub Actions publish workflow, `NPM_TOKEN` secret, provenance requirements, tag process, and verification commands.
- Records known failure modes: `E403`, `EOTP`, and provenance `E422`.
- Updates README and installation docs to use `npm install -g canvas-design-mcp` instead of GitHub install.
- Updates `docs/feature-roadmap.md`, which still said npm publish was the next milestone.

Do not document or expose the actual npm token value.
...
1. **Live testing** — test the full professor workflow end-to-end with a real Canvas course
2. **Docker runtime validation** — pull and run `ghcr.io/ryfter/canvas-design-studio:latest` on Windows and macOS
3. **Release maintenance** — use `docs/npm-publishing.md` for future npm/Docker version tags
...
`package.json`, `package-lock.json`, and `AGENTS.md` were also moved from `0.1.0` to `0.9.0`. Reasoning: the existing public `v0.1.0` tag points at the old SP1 initial-release commit, while the completed application is documented through v0.9/SP9. This was later published successfully; current `latest` is `canvas-design-mcp@0.9.5`.
...
No SP1-SP9 application work remains. npm and Docker publishing are configured and live at `v0.9.5`. Live Canvas testing and Docker runtime testing remain prudent post-release validation tasks.
